...
std::string fourColorSolver::loadFromFile(std::string &fileName) {
    std::ifstream inFile;
...
bool fourColorSolver::bruteForceHelper(int n, time_point start) {
    // check timeout
...
bool fourColorSolver::bruteForce() {
...
std::string fourColorSolver::solveGraph() {
    if (heuristic()) {
        if (!checkSolution()) {
        ...
        return "Timeout or Cannot be Solved";
    }
}